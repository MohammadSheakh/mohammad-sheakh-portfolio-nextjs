...
const fieldClassName =
  "mt-2 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-cyan-300 dark:focus:ring-cyan-900/40";
...
export default function ArticleReader({
  ...
  return (
    <main className="min-h-screen bg-[#eef8fb] px-4 pb-12 pt-28 text-slate-900 dark:bg-PrimaryColorDark dark:text-slate-100 md:px-8">
      <div className={`mx-auto grid max-w-7xl gap-5 ${editing ? "lg:grid-cols-[390px_minmax(0,1fr)]" : "lg:grid-cols-[330px_minmax(0,1fr)]"}`}>
        ...
        <aside className="self-start rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:sticky lg:top-28 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto">
          <Link className="text-xs font-bold uppercase tracking-[1.5px] text-cyan-700 dark:text-cyan-300" href="/articles">
            ← All articles
          </Link>

          {editing ? (
            <details className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950/60">
            ...
            <div className="mt-5 rounded-2xl bg-PrimaryColorDark p-6 text-white dark:border dark:border-slate-700">
          ...
          <nav className="mt-6" aria-label="Article pages">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              {editing && <button className="rounded-lg bg-cyan-100 px-3 py-1.5 text-xs font-bold text-cyan-900 dark:bg-cyan-900/40 dark:text-cyan-200" type="button" onClick={addPage}>+ New page</button>}
            </div>
            <div className="space-y-2">
              {article.pages.map((page, pageIndex) => (
                <div className="rounded-xl border border-transparent bg-slate-50/70 p-1 dark:bg-slate-950/40" key={page.id}>
                  <div className="flex items-center gap-1">
                    <button type="button" className={`min-w-0 flex-1 rounded-lg px-3 py-2 text-left text-sm font-semibold transition-colors ${selectedId === page.id ? "bg-cyan-100 text-cyan-900 dark:bg-cyan-900/40 dark:text-cyan-200" : "text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"}`} onClick={() => selectPage(page.id)}>
                      <span className="block truncate">{page.title}</span>
                      {page.isIndex && <span className="text-[0.6rem] uppercase tracking-wider text-cyan-700 dark:text-cyan-300">Section index</span>}
                    </button>
                    {editing && (
                      <>
                        <button className="rounded-md px-2 py-2 text-sm text-cyan-700 hover:bg-cyan-50 dark:text-cyan-300 dark:hover:bg-cyan-950/40" type="button" title="Add subpage" onClick={() => addSubPage(pageIndex)}>+</button>
                        <button className="rounded-md px-2 py-2 text-sm text-rose-600 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-950/30" type="button" title="Remove page" onClick={() => removePage(pageIndex)}>×</button>
                      </>
                    )}
                  </div>
                    ...
                    <div className="ml-4 mt-1 border-l border-slate-200 pl-2 dark:border-slate-700">
                      {page.subPages.map((subPage, subPageIndex) => (
                        <div className="flex items-center gap-1" key={subPage.id}>
                          <button type="button" className={`min-w-0 flex-1 rounded-lg px-3 py-2 text-left text-sm transition-colors ${selectedId === subPage.id ? "bg-cyan-50 font-semibold text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-200" : "text-slate-500 hover:bg-white hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white"}`} onClick={() => selectPage(subPage.id)}>
                            <span className="block truncate">{subPage.title}</span>
                          </button>
                          {editing && <button className="rounded-md px-2 py-2 text-sm text-rose-600 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-950/30" type="button" title="Remove subpage" onClick={() => removeSubPage(pageIndex, subPageIndex)}>×</button>}
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {adminState === "admin" && (
            <div className="mt-6 grid grid-cols-2 gap-2">
              {editing ? (
                <>
                  <button className="rounded-xl border border-slate-300 px-4 py-3 text-xs font-bold uppercase tracking-[1px] text-slate-700 dark:border-slate-600 dark:text-slate-200" type="button" onClick={cancelEditing}>Cancel</button>
                  <button className="rounded-xl bg-slate-950 px-4 py-3 text-xs font-bold uppercase tracking-[1px] text-white disabled:opacity-40 dark:bg-cyan-300 dark:text-slate-950" type="button" disabled={saving} onClick={saveArticle}>{saving ? "Saving..." : "Save all"}</button>
                </>
              ) : (
                <button className="col-span-2 rounded-xl bg-slate-950 px-4 py-3 text-xs font-bold uppercase tracking-[1px] text-white dark:bg-cyan-300 dark:text-slate-950" type="button" onClick={() => setEditing(true)}>Edit article</button>
              )}
            </div>
          )}
          {error && <p className="mt-3 rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-700 dark:bg-rose-950/30 dark:text-rose-300">{error}</p>}
        </aside>

        {/* Only the selected page document is shown or edited. */}
        <article className="min-h-[720px] rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 md:p-10 lg:p-14">
          {!editing && (
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-6 dark:border-slate-700">
              <div className="text-xs text-slate-400">
                {selectedPage?.parentTitle ? `${selectedPage.parentTitle} / ` : ""}
                <span className="font-semibold text-slate-700 dark:text-slate-200">{selectedPage?.title}</span>
              </div>
          ...
          {selectedPage && editing ? (
            <div>
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-5 dark:border-slate-700">
                <div>
                  <div className="text-[0.68rem] font-bold uppercase tracking-[2px] text-cyan-700 dark:text-cyan-300">Editing selected document</div>
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Choose another page from the sidebar without losing this draft.</p>
                </div>
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">{selectedPage.subPageIndex === undefined ? "Main page" : "Subpage"}</span>
              </div>
              ...
              {selectedPage.subPageIndex === undefined && (
                <label className="mt-5 flex items-start gap-3 rounded-xl border border-cyan-200 bg-cyan-50 p-4 text-sm text-slate-700 dark:border-cyan-900 dark:bg-cyan-950/40 dark:text-slate-300">
                  <input className="mt-0.5 size-4 accent-cyan-700 dark:accent-cyan-300" type="checkbox" checked={selectedPage.isIndex} onChange={(event) => updateSelectedPage("isIndex", event.target.checked)} />
                  <span><strong className="block text-slate-950 dark:text-white">This page is a section index</strong>Use it only to organize subpages; no main-page content is required.</span>
                </label>
              )}
              {selectedPage.isIndex ? (
                <div className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-slate-700 dark:bg-slate-950/40">
                  <h3 className="font-display text-2xl font-black text-slate-900 dark:text-white">Index page</h3>
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Add and select subpages from the sidebar to write their documents.</p>
                  <button className="mt-5 rounded-xl bg-cyan-100 px-4 py-2 text-sm font-bold text-cyan-900 dark:bg-cyan-900/40 dark:text-cyan-200" type="button" onClick={() => addSubPage(selectedPage.pageIndex)}>+ Add subpage</button>
                </div>
              ) : (
                <div className="mt-6">
            ...
            <div className="mt-10">
              <div className="mb-3 text-[0.68rem] font-bold uppercase tracking-[2px] text-cyan-700 dark:text-cyan-300">Page {selectedIndex + 1} of {readerPages.length}</div>
              <h2 className="font-display text-[clamp(2.2rem,5vw,4rem)] font-black leading-none tracking-[-2px] text-slate-950 dark:text-white">{selectedPage.title}</h2>
              {selectedPage.isIndex ? (
                <div className="mt-10">
                  <p className="text-slate-600 dark:text-slate-300">Choose a page from this section:</p>
                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    {selectedParent?.subPages.map((subPage, index) => (
                      <button className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left transition hover:border-cyan-300 hover:bg-cyan-50 dark:border-slate-700 dark:bg-slate-950/40 dark:hover:border-cyan-700 dark:hover:bg-cyan-950/30" type="button" key={subPage.id} onClick={() => selectPage(subPage.id)}>
                        <span className="text-xs font-bold text-cyan-700 dark:text-cyan-300">{String(index + 1).padStart(2, "0")}</span>
                        <span className="mt-2 block font-display text-xl font-black text-slate-900 dark:text-white">{subPage.title}</span>
                      </button>
                    ))}
                  </div>
                  {selectedParent?.subPages.length === 0 && <p className="mt-6 rounded-xl bg-slate-50 p-5 text-sm text-slate-500 dark:bg-slate-950/40 dark:text-slate-400">No subpages have been added yet.</p>}
                </div>
              ) : (
                <div className="mt-10 text-base leading-8 text-slate-700 dark:text-slate-300 [&_blockquote]:my-7 [&_blockquote]:border-l-4 [&_blockquote]:border-cyan-500 [&_blockquote]:bg-cyan-50 [&_blockquote]:px-6 [&_blockquote]:py-4 [&_blockquote]:italic dark:[&_blockquote]:bg-cyan-950/30 [&_font]:font-mono [&_h1]:mb-5 [&_h1]:font-display [&_h1]:text-4xl [&_h1]:font-black [&_h1]:text-slate-950 dark:[&_h1]:text-white [&_h2]:mb-4 [&_h2]:mt-8 [&_h2]:font-display [&_h2]:text-3xl [&_h2]:font-black [&_h2]:text-slate-950 dark:[&_h2]:text-white [&_h3]:mb-3 [&_h3]:mt-7 [&_h3]:text-2xl [&_h3]:font-bold [&_h3]:text-slate-950 dark:[&_h3]:text-white [&_li]:mb-2 [&_ol]:my-5 [&_ol]:list-decimal [&_ol]:pl-7 [&_p]:mb-5 [&_pre]:my-6 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-slate-950 [&_pre]:p-5 [&_pre]:font-mono [&_pre]:text-sm [&_pre]:text-cyan-100 dark:[&_pre]:bg-black [&_ul]:my-5 [&_ul]:list-disc [&_ul]:pl-7" dangerouslySetInnerHTML={{ __html: sanitizeRichText(selectedPage.contentHtml) }} />
              )}
            </div>
          ) : null}
        </article>
